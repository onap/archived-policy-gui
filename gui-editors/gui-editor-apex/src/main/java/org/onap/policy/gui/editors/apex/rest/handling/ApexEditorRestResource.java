...
public class ApexEditorRestResource implements RestCommandHandler {
    ...
    public ApexEditorRestResource() {
        policyUploadHandler = new PolicyUploadHandler(
            new UploadPluginClient(), new PolicyToscaConverter(new StandardCoder()),
            new ToscaTemplateProcessor(), new ApexConfigProcessor(), ConfigProvider.getConfig()
        );
    }
     ...
     * messages/errors can be retrieved using {@link ApexApiResult#getMessages()}. This includes the session id for this
     * session.
     */
    @GET
    @Path("Session/Create")
