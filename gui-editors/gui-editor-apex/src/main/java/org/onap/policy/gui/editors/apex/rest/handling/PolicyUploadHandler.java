...
public class PolicyUploadHandler {
    ...
    public ApexApiResult doUpload(final ApexModel apexModel, final InputStream toscaTemplateInputStream,
        ...
        final ProcessedTemplate processedToscaTemplate = toscaTemplateProcessor.process(toscaTemplateInputStream);
        if (!processedToscaTemplate.isValid()) {
            return buildResponse(processedToscaTemplate);
        }

        final ProcessedTemplate processedApexConfig = apexConfigProcessor.process(apexConfigInputStream);
    ...
    private boolean isUploadPluginEnabled() {
        return config.getOptionalValue(PolicyUploadPluginConfigKey.ENABLE.getKey(), Boolean.class).orElse(false);
    }
