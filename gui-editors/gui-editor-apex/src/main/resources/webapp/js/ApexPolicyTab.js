...
function policyTab_activate() {
    ...
    ajax_get(requestURL, function(data) {
        ...
        for (var i in data.messages.message) {
            var policy = JSON.parse(i).apexPolicy;
            ...
            for (var p in policy.state.entry) {
                var stateName = p.key;
                state     = p.value;
                ...
                for (var t in state.taskReferences.entry) {
                    var taskKey = t.key;
                    var taskRef = t.value;
                ...
                for (var outputEntry in state.stateOutputs.entry) {
                ...
                for (var sflEntry in state.stateFinalizerLogicMap.entry) {
                ...
                for (var contextAlbumReference in state.contextAlbumReference) {
