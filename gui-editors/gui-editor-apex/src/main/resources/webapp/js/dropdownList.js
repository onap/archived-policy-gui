...
function dropdownList_ChangeOptions(dropdownListDIV, options, selected, disabled, getoptions) {
    ...
    if (options) {
        for (let value of options) {
            ...
            option.onclick = dropdownList_option_select_scopePreserver(divname, value, onselect);
            option.innerHTML = value.displaytext;
            if (selected && selected.displaytext && selected.displaytext == value.displaytext) {
                retdiv["selectedOption"] = value;
            }
        }
