...
function editPolicyForm_activate(parent, operation, policy, tasks, events, contextAlbums, contextItemSchemas) {
    ...
    if (policy != null && policy.firstState != null && policy.firstState != "" && policy.state != null) {
        for (let i of policy.state.entry) {
    ...
    triggerPeriodicEventCheckbox.onclick = function() {
        ...
        if ($(triggerPeriodicEventCheckbox).is(":checked")) {
            ...
            for (let e of events) {
                if (e.name.indexOf("PeriodicEvent") !== -1) {
                    periodicEvent = e;
                    break;
                }
            }
...
function editPolicyForm_getStateOptions() {
    ...
    var stateslis = document.getElementById("editEventFormStates").querySelectorAll(
            "#editEventFormStates > li[stateName]"); // get li direct child
                                                        // elements with an
                                                        // attribute "stateName"
    for (let state of stateslis) {
        if (state != null && state.getAttribute("stateName") != null) {
            stateoptions.push({
                "name" : state.getAttribute("stateName"),
                "displaytext" : state.getAttribute("stateName")
            });
        }
    }
    return stateoptions;
...
function editPolicyForm_getPolicyBean() {
    ...
    for (let li of stateslis) {                       // ignore last li ... it has
        ...
        if (li != null && li.getAttribute("stateName") != null) {
            let statename = li.getAttribute("stateName");
            let state = editPolicyForm_State_getStateBean(statename);
            if (state == null) {
                return null;
            }
