...
function pageControl_successStatus(data) {
    ...
    if (data.ok) {
        ...
        for (var msg in data.messages.message) {
            $("#statusMessageTable").append("<tr><td>" + msg + "</td></tr>");
        }
        ...
        throw new Exception("REST call returned an error\n" + data);
    }
}
...
function pageControl_recursiveDisable(el, disableValue, visibleValue) {
    ...
    if (el.childNodes && el.childNodes.length > 0) {
        for (var node in el.childNodes) {
            recursiveDisable(node, disableValue, visibleValue);
        }
    }
}
