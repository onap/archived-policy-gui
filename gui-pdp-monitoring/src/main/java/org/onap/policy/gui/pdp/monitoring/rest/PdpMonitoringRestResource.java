...
public class PdpMonitoringRestResource {
    ...
    public Response getStatistics(@QueryParam("useHttps") final String useHttps,
        ...
        if (idArray.length == 3) {
            ...
            throw new IllegalArgumentException("Cannot parse groupName, subGroup and instanceId from " + idArray);
        }
        ...
        // Engine Service data
        responseObject.addProperty("engineId", pdp.getInstanceId());
        ...
        if (!pdpStats.isEmpty()) {
            ...
            for (final PdpEngineWorkerStatistics engineStats : pdpStatistics.getEngineStats()) {
                try {
                    ...
                    engineStatusObject.addProperty("lastMessage",
                            new Date(engineStats.getEngineTimeStamp()).toString());
                    engineStatusObject.addProperty("upTime", engineStats.getUpTime());
                    engineStatusObject.addProperty("policyExecutions", engineStats.getEventCount());
                    engineStatusObject.addProperty("lastPolicyDuration",
                            gson.toJson(
                                    ...
                                    List.class));
                    engineStatusObject.addProperty("averagePolicyDuration",
    ...
    private synchronized List<Counter> getValuesFromCache(final String uri, final String id, final long timestamp,
        ...
        Map<String, List<Counter>> engineStatus = cache.computeIfAbsent(uri, k -> new HashMap<String, List<Counter>>());
