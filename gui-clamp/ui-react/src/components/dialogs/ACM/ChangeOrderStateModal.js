...
const ChangeOrderStateModal = (props) => {
  ...
  useEffect(async () => {
    ...
    if (!instantiationOrderState.ok || orderStateJson['acmIdentifierList'].length === 0) {
      setInstantiationOrderStateError(true);
      setInstantiationOrderStateMsgError(orderStateJson);
    } else {
      setACMIdentifierList(orderStateJson['acmIdentifierList']);
  ...
  const handleDropSelect = (event) => {
    ...
    const stateChangeObject = {
      orderedState: event,
      ACMIdentifierList: ACMIdentifierList
    }
